...
@Service
@Scope("prototype")
@Repository("dataLoadEmployeeProjectExecutorServImpl")
public class DataLoadEmployeeProjectExecutorServImpl implements Runnable {
	
	private static final Logger log =  Logger.getLogger(DataLoadEmployeeProjectExecutorServImpl.class);
	...
	@Resource
	private EmployeeServ projectServ;
	...
	private void process() {
		try {
			...
			List<DeleteEmployeeProject> deleteProjectLocationList = entityServiceDBaaS.findByNameQuery("GET_UNPROCESSED_EMPLOYEE_PROJECT_FOR_DELETE",new Object[]{sync_status});
			while (deleteProjectLocationList != null &&  deleteProjectLocationList.size() > 0) {
				for (DeleteEmployeeProject o : deleteProjectLocationList){
					try {
						...
						o.setErrorMessage(CommonUtil.getExceptionMessageWithCauses(e));
						entityServiceDBaaS.save(o);
						log.error(e);
					}
				}
				deleteProjectLocationList=null;
				deleteProjectLocationList = entityServiceDBaaS.findByNameQuery("GET_UNPROCESSED_EMPLOYEE_PROJECT_FOR_DELETE",new Object[]{sync_status});
			}
	 ...
	 * @throws Exception
	 */
	private void delete(DeleteEmployeeProject accountFromDBAS) throws Exception {
		try {
			log.info("deleting...");
