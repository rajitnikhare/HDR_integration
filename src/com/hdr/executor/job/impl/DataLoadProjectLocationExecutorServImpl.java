...
@Service
@Scope("prototype")
@Repository("dataLoadProjectLocationExecutorServImpl")
public class DataLoadProjectLocationExecutorServImpl implements Runnable {
	
	private static final Logger log =  Logger.getLogger(DataLoadProjectLocationExecutorServImpl.class);
	...
	private void process() {
		try {
			...
			List<DeleteProjectLocation> deleteProjectLocationList = entityServiceDBaaS.findByNameQuery("GET_UNPROCESSED_PROJECT_LOCATION_FOR_DELETE",new Object[]{sync_status});
			while (deleteProjectLocationList != null &&  deleteProjectLocationList.size() > 0) {
				for (DeleteProjectLocation o : deleteProjectLocationList){
					try {
				...
				deleteProjectLocationList = entityServiceDBaaS.findByNameQuery("GET_UNPROCESSED_PROJECT_LOCATION_FOR_DELETE",new Object[]{sync_status});
			}
	 ...
	 * @throws Exception
	 */
	private void delete(DeleteProjectLocation accountFromDBAS) throws Exception {
		try {
			log.info("deleting...");
